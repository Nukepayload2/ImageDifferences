...
Public Class ImageDiffBlendCalc
    ...
    Sub New()
        ...
        Using sr As New StreamReader([GetType].GetTypeInfo.Assembly.GetManifestResourceStream("ImgDiff.NetCore2.ImageDiffBlendCalc.cl"), Encoding.UTF8)
    ...
    Public Function Invoke(imageA As Bitmap, imageB As Bitmap) As Bitmap Implements IImageProc.Invoke
        ...
        Using cl = _method.CreateCommandList
            cl.LoadInput(imageAData.Scan0, imageAData.Stride * imageAData.Height)
            cl.LoadInput(imageBData.Scan0, imageBData.Stride * imageBData.Height)
            cl.LoadOutput(imageCData.Scan0, imageCData.Stride * imageCData.Height)
            cl.Call(4)
        End Using
    ...
    ''' <summary>
    ''' 比较两张大小一样的 Format32bppArgb 位图（无硬件加速）。
    ''' </summary>
    Public Function NoHardwareAcceleration(imageA As Bitmap, imageB As Bitmap) As Bitmap Implements IImageProc.NoHardwareAcceleration
        ' A
        Dim inputImgWidth = imageA.Width
        Dim inputImgHeight = imageA.Height
        If imageA.PixelFormat <> PixelFormat.Format32bppArgb Then
        ...
        ' 调用
        ImgDiff.NoOpenCL.ImageComparer.Compare(imageAData.Height * imageAData.Stride, imageAData.Scan0, imageBData.Scan0, imageCData.Scan0)
